...
def dawn_cfg(c=64, block=res_block, prep_bn_relu=False, concat_pool=True, **kw):
    if isinstance(c, int):
        c = [c, 2 * c, 4 * c, 4 * c]
    ...
    return {
        ...
        "prep": union(
            ...
            {"bn": batch_norm(c[0], **kw), "relu": nn.ReLU(True)}
            if prep_bn_relu
            else {},
        ),
        "layer1": {
...
def basic_net(channels, weight, pool, **kw):
    return {
        ...
        "layer2": dict(
            conv_bn(channels["layer1"], channels["layer2"], **kw), pool=pool
        ),
        "layer3": dict(
            conv_bn(channels["layer2"], channels["layer3"], **kw), pool=pool
        ),
        "pool": nn.MaxPool2d(4),
        "flatten": Flatten(),
...
def pipeline(net):
    return [
        ("/".join(path), (node if has_inputs(node) else (node, [-1])))
