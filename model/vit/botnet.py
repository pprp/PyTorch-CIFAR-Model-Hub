...
"""
from https://github.com/leaderj1001/BottleneckTransformers
"""
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        self, in_planes, planes, stride=1, heads=4, mhsa=False, resolution=None
    ):
...
class ResNet(nn.Module):
    def __init__(
        self, block, num_blocks, num_classes=1000, resolution=(224, 224), heads=4
    ):
        ...
        self.layer4 = self._make_layer(
            block, 512, num_blocks[3], stride=2, heads=heads, mhsa=True
        )
        ...
        self.fc = nn.Sequential(
            nn.Dropout(
                0.3
            ),  # All architecture deeper than ResNet-200 dropout_rate: 0.2
    ...
    def _make_layer(self, block, planes, num_blocks, stride=1, heads=4, mhsa=False):
        ...
        for idx, stride in enumerate(strides):
            layers.append(
                block(self.in_planes, planes, stride, heads, mhsa, self.resolution)
            )
            if stride == 2:
                self.resolution[0] /= 2
                self.resolution[1] /= 2
