...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--name", default=None, help="model name: (default: cifar10_ricap)"
    )
    parser.add_argument(
        "--dataset",
        default="cifar10",
    ...
    train_dataloader, num_classes = build_dataloader("cifar10", "train", args=args)
    ...
    optimizer = optim.SGD(
        params, lr=args.base_lr, momentum=0.9, weight_decay=1e-4, nesterov=True
    )
    ...
    for epoch in range(epoches):
        ...
        for batch_index, (images, labels) in enumerate(train_dataloader):
            ...
            print(
                "Iterations: {iter_num} [{trained_samples}/{total_samples}]\tLoss: {:0.4f}\tLR: {:0.8f}".format(
                    loss.item(),
